...
test.describe('Price Chart', () => {
  test('chart renders with token pair info', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="price-chart"]').or(
      page.locator('svg').filter({ has: page.locator('path') })
    ).first()).toBeVisible({ timeout: 10000 })

    // Time presets should be available
    const timeButtons = page.locator('button').filter({ hasText: /^(1H|4H|1D|1W|ALL)$/ })
  ...
  test('chart time preset buttons are interactive', async ({ page }) => {
    ...
    const presetButtons = page.locator('button').filter({ hasText: /^(1H|4H|1D|1W|ALL)$/ })
  ...
  test('chart displays price headline when data available', async ({ page }) => {
    await page.goto('/swap')
    
    // Should display some price-related text
    // Either current price, loading state, or error state
    const priceArea = page.locator('[class*="chart"]').or(
      page.locator('text=/\\$[0-9.,]+|Loading|No data/')
    )
...
test.describe('Quote Details Card', () => {
  ...
  test('price impact row shows placeholder without input', async ({ page }) => {
    ...
    const priceImpactValue = page.locator('div').filter({ hasText: /^Price impact/ }).locator('span').last()
  ...
  test('slippage row shows current setting', async ({ page }) => {
    ...
    await expect(page.getByText('Max. slippage', { exact: true })).toBeVisible()
...
test.describe('Settings Modal Price Impact', () => {
  test('settings modal has price impact warning section', async ({ page }) => {
    ...
    const slippageButton = page.locator('button').filter({ hasText: /[0-9.]+%/ }).first()
  ...
  test('all three settings sections are visible in grid', async ({ page }) => {
    ...
    await expect(page.locator('p').filter({ hasText: 'Slippage' }).first()).toBeVisible()
    await expect(page.locator('p').filter({ hasText: 'Deadline' }).first()).toBeVisible()
    await expect(page.locator('p').filter({ hasText: 'Impact Warning' }).first()).toBeVisible()
  })
