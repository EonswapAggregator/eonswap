...
test.describe('Liquidity Page (/liquidity)', () => {
  ...
  test('displays stats section', async ({ page }) => {
    await expect(page.getByText(/active pools|total liquidity|your positions/i).first()).toBeVisible()
  })
...
test.describe('Stake Page (/stake)', () => {
  ...
  test('displays hero section', async ({ page }) => {
    await expect(page.getByText(/single-sided staking|secure/i).first()).toBeVisible()
  })
  ...
  test('xESTF Staking panel visible', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'xESTF Staking' })).toBeVisible()
  })

  test('veESTF Voting panel visible', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'veESTF Voting' })).toBeVisible()
  })
  ...
  test('info section has links to farm and liquidity', async ({ page }) => {
    const infoSection = page.locator('section').last()
    await expect(infoSection.getByRole('link', { name: /farm/i })).toBeVisible()
    await expect(infoSection.getByRole('link', { name: /liquidity/i })).toBeVisible()
  })
})
