...
const TIER_THRESHOLDS: Record<ReferralTier, { min: number; rewardPct: number }> = {
 ...
 * Falls back to localStorage when API is unavailable.
 */
export async function loadReferralStats(address: string): Promise<ReferralStats> {
  const code = generateReferralCode(address)
...
export async function registerReferral(
  ...
  if (USE_API) {
    ...
    if (base) {
      try {
        ...
        const json = await res.json()
        if (res.ok && json.ok) {
          return { success: true, message: json.message || 'Referral registered' }
        }
        if (json.duplicate) {
  ...
  // Fallback: localStorage
  try {
    const stored = localStorage.getItem(`${REFERRAL_STORAGE_KEY}_${referrerCode}`)
    ...
    localStorage.setItem(`${REFERRAL_STORAGE_KEY}_${referrerCode}`, JSON.stringify(stats))
...
export async function trackReferralSwap(
  ...
  if (!address || !volumeUsd) return { success: false }
...
export function saveReferralStats(code: string, stats: ReferralStats): void {
  try {
    localStorage.setItem(`${REFERRAL_STORAGE_KEY}_${code}`, JSON.stringify(stats))
  } catch {
