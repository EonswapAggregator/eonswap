...
export const EON_REFERRAL_ABI = [
  ...
  },
  {
    type: 'function',
    name: 'calculateTier',
    inputs: [{ name: 'referralCount', type: 'uint256', internalType: 'uint256' }],
    outputs: [{ name: '', type: 'uint8', internalType: 'enum EonReferral.Tier' }],
    stateMutability: 'view',
  },
  {
    type: 'function',
    ...
    outputs: [
      ...
      { name: 'nextTier', type: 'uint8', internalType: 'enum EonReferral.Tier' },
...
export const EON_REFERRAL_ADDRESS: Record<number, `0x${string}`> = {
  // Base Mainnet
  8453: '0x0000000000000000000000000000000000000000', // TODO: Update after deployment
  // Base Sepolia
 ...
 */
export function getEonReferralAddress(chainId: number): `0x${string}` | undefined {
 ...
 */
export function getTierName(tier: Tier): 'bronze' | 'silver' | 'gold' | 'platinum' {
  const names: readonly ['bronze', 'silver', 'gold', 'platinum'] = ['bronze', 'silver', 'gold', 'platinum'] as const
